...
pub fn execute(modules_dir: &Path, registry_path: &Path, module_name: Option<String>, args: Vec<String>) -> i32 {
    let module_name = match module_name {
        Some(m) => m,
        None => {
            println!("Error: module name required");
            println!("Usage: aktools run <module> [args...]");
            println!("Or use an alias defined in your shell.");
            1
        }
    };
    ...
    if !module_path.exists() {
        ...
        match cmd.spawn() {
            Ok(mut child) => child.wait().map(|s| s.exitcode()).unwrap_or(1),
            Err(e) => {
